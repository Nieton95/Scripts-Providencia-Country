...
#---------------------------------------------------------------------------------
# 2. ANÁLISIS DE NORMALIDAD (RQD)
#---------------------------------------------------------------------------------

rqd_vals <- RQD_utm_sp$RQD

# Histograma
p_hist_norm <- ggplot(data.frame(RQD = rqd_vals), aes(x = RQD)) +
  geom_histogram(bins = 20, fill = "steelblue", color = "white", alpha = 0.8) +
  labs(
    title = "Histograma – RQD",
    x = "RQD (%)",
    y = "Frecuencia"
  ) +
  theme_minimal()
...
# QQ-plot
p_qq_norm <- ggplot(data.frame(RQD = rqd_vals), aes(sample = RQD)) +
...
# Prueba Shapiro-Wilk
shapiro_test <- shapiro.test(rqd_vals)

cat("\n--- PRUEBA DE NORMALIDAD (Shapiro-Wilk) – RQD ---\n")
print(shapiro_test)
...
#--------------------------------------------------------------------------------
# 10. ANÁLISIS DE SENSIBILIDAD (variación de nmax) – RQD
#--------------------------------------------------------------------------------
...
cat("\n--- ANÁLISIS DE SENSIBILIDAD (nmax) – RQD ---\n")
print(sens_results)
...
p_sens <- ggplot(sens_results, aes(x = nmax)) +
  ...
  labs(
    title = "Análisis de sensibilidad – RMSE vs nmax (RQD)",
