...
bbox <- st_bbox(RQD_var)  # Obtener los límites actuales de los datos
...
kriging_result_expanded <- krige(RQD ~ 1, RQD_var, grid_expanded, model = modelo_variograma, nmax = 1)  # Aplicar kriging con nmax=10 para una extrapolación más suave

# Convertir los resultados a un data frame para ggplot2
kriging_df_expanded <- as.data.frame(kriging_result_expanded)
colnames(kriging_df_expanded) <- c("Long", "Lat", "RQD")  # Renombrar columnas para ggplot2
kriging_df_expanded <- na.omit(kriging_df_expanded)


# Redondear valores para mejorar la visualización
kriging_df_expanded$RQD <- round(kriging_df_expanded$RQD, 2)

RQD_var <- as.data.frame(RQD_var)
...
kriging_raster <- rasterFromXYZ(kriging_df_expanded[, c("Long", "Lat", "RQD")])
